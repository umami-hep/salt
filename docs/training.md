...
- That there are no `nan` inputs (see the reader in `salt.data`)
- That your normalisation paramters are finite (see `salt.model.modules.Normaliser`)
...
- If you apply very large loss weights in your task configs, these might contribute to large gradients, so you can try removing any loss weights provided to your task modules (`salt.model.modules.tasks`).
