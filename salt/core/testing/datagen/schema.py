...
def _propagate_required_match_min_valid(schema: Schema) -> None:
    ...
    Policy (design §3.5, resolved): a ``link`` field with ``required_match: true``
    that references a CONSTITUENT group ``G`` raises the effective
    ``G.min_valid`` to ``max(G.min_valid, 1)``. This forces phase 1 to generate
    at least one genuinely-valid referent (real id from the id field's range,
    real payload) in every sample, so the phase-2 resolver always finds a
    non-empty pool. The alternative -- promoting an already-invalid-filled slot
    to ``valid`` at phase 2 -- is wrong: that slot's id has already been set to
    the int fill sentinel, producing a "valid" referent with a sentinel id.
