...
pipeline and assigned to ``self._schema`` before ``__call__``; design §2.4-2.5,
§6 decided default 2). This keeps ``write_h5``'s class_names attrs and the
``compute_class_dict`` length == head ``output_size`` guarantee intact.
"""
