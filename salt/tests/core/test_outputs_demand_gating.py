...
GATES (a)/(b) reuse the GN2 forward-parity fixture (`salt.core.parity_gn2`):
the producer is wired onto a real task's ``preds.*`` leaf, so the proof runs
against the production GN2 topology, not a toy. GATE (c) and a kernel-level
pruning control use minimal stub `GraphModule`s for a focused contract test.
"""
...
from salt.core.parity_gn2 import run_parity
