"""Kernel tests for the setup-graph (plan-24 §3/§4, plan-25 §3.6 / W3.0).

Covers the W3.0 deliverables: the `SourceSpec`/`SetupIO` type system, the
`compile_setup_plan` planner reuse (topo + validation, shape-unification
SKIPPED), the bespoke setup-execution loop (write-once ctx threading), and the
namespace split that keeps setup-only modules OUT of the per-batch compile (the
`AllModesDeadError` blocker). The existing tensor `compile_plan` is exercised
as a control to prove the `_compile_core` refactor did not perturb it.
"""
