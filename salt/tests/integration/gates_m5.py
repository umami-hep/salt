...
Standalone gates, each a subcommand of ``python -m salt.tests.integration.gates_m5``, each
...
in-repo parity generators (`salt.tests._fixtures.regression_fixture` + the gn2
...
from salt.tests._fixtures.gn2v2_fixture import ORIGIN_CLASSES, build_gn2v2_modules, gn2v2_sources
...
def _write_probe_writer_config(outdir: Path, *, kind: str, dtype: str | None, fname: str) -> Path:
    ...
    (``salt.tests.integration.gates_m5._SpecProbeWriter``) is wired into ``writers.modules``
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="python -m salt.tests.integration.gates_m5", description=__doc__.splitlines()[0]
    )
