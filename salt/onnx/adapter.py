...
class OnnxAdapter(nn.Module):
    ...
    def dynamic_axes(self) -> dict[str, dict[int, str]]:
        """Dynamic-axes mapping for ``torch.onnx.export`` (sequence inputs + per-token outputs)."""
    ...
    def forward(self, *args: Tensor) -> tuple[Tensor, ...]:
        ...
        `*args`: tensors in ``export.inputs`` positional order (alias
        entries consume none), globals ``[1, F]``, sequences ``[L, F]``
        (Athena-style, no batch dim). Returns outputs in `output_names` order.
        """
    ...
    def _field_list(self, port: str) -> tuple[str, ...]:
        """The declared columns of an ``inputs.<stream>`` port; raises
        `ConfigError` when the port has no `Features` declaration.
        """
        try:
    ...
    def _resolve_alias_gather(self, entry: ExportInput) -> Tensor | None:
        ...
        otherwise a name-resolved ``index_select`` gather (int64 indices)
        with bind-time constant indices. Raises `ConfigError` when a target
        column is missing from the alias source.
        """
