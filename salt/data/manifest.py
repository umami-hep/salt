...
def main() -> int:
    ...
    Pre-builds the train-stage manifest offline so no training job pays for it.
    Largely redundant with build-on-missing in the datamodule: a streaming
    config already names its manifest path, and a run builds a missing one
    there. This exists for the case where even the first build must not happen
    inside a job.
    """
    import argparse
    ...
    manifest = build_manifest(dataset.reader, meta={"stage": "train"})
